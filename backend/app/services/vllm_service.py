...
class VLLMService:
    ...
    def __init__(self):
        ...
        # Configuration du modèle de vision pour le mode local
    ...
    async def _sync_vision_model(self, client: httpx.AsyncClient) -> None:
        """Ensure configured vision model exists on the local vLLM server."""
        try:
            ...
            if self.vision_model not in available_models:
                message = (
                    f"Le modèle de vision configuré '{self.vision_model}' "
                    f"n'est pas disponible sur le serveur local. "
                    f"Modèles disponibles: {available_models}"
                )
                logger.error(message)
                raise ExternalServiceError("Vision vLLM", Exception(message))
    ...
    async def process_image_with_vision_model(self, image_base64: str, prompt: str = "Décris cette image en détail.") -> str:
        """
        Traiter une image avec le modèle de vision configuré en mode local (vLLM).
        ...
        Returns:
            str: Description ou réponse du modèle de vision
        """
        headers = {"Content-Type": "application/json"}
        
        # Construire le payload selon le format attendu par un serveur vLLM compatible vision
        payload = {
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                try:
                    ...
                    raise ExternalServiceError(
                        "Vision vLLM",
                        Exception(
                            "Impossible de se connecter au serveur de vision local. "
    ...
    async def vision_health_check(self) -> bool:
        """Vérifier que le serveur de vision vLLM est accessible"""
        try:
